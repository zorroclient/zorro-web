...
// Minimal typing for the tables this client touches, so `.from()` is typed
...
type DesktopSessionRow = {
  ...
  payload_object_path: string;
  payload_hash: string;
  engine_token_jti: string;
  issued_at: string;
  ...
  public: {
    Tables: {
      ...
      desktop_sessions: {
        ...
        Insert: Partial<DesktopSessionRow> & {
          id: string;
          user_id: string;
          fingerprint_hash: string;
        };
...
// Privileged Supabase client for trusted, server-only code. Uses the Supabase
// Secret key (sb_secret_…), which bypasses RLS, so
